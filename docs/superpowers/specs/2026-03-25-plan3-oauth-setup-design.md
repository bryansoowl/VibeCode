...
- On completion: writes `.env`, prints success message with `npm run build && npm start`
- Skipped providers write empty strings (matching `.env.example` format — app still starts)
...
─── Outlook Setup ──────────────────────────
  ...
  3. Name: InboxMY, Accounts: Personal Microsoft accounts
...
- Add a new `validateConfig()` function called immediately after `dotenv.config()`
...
| Invalid Client ID format | Prints format hint, re-prompts (does not write partial `.env`) |
...
| `inboxmy-backend/src/config.ts` | Add `validateConfig()`, call on startup |
