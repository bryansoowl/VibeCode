...
- On completion: always writes a **complete `.env`** to `path.join(process.cwd(), '.env')` — the canonical invocation is `npm run setup` from `inboxmy-backend/`, which sets `cwd` correctly. Running `tsx scripts/setup.ts` from the repo root would land `.env` in the wrong directory; the spec does not support that path. The written file contains all known variables (`PORT`, `DATA_DIR`, `ENCRYPTION_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, `MICROSOFT_CLIENT_ID`, `MICROSOFT_CLIENT_SECRET`, `MICROSOFT_REDIRECT_URI`, `SYNC_INTERVAL_MINUTES`). Skipped providers write empty strings. Non-credential values (`PORT`, `DATA_DIR`, `SYNC_INTERVAL_MINUTES`) are always written with their defaults. **Note:** if the user has previously customized `PORT` or `DATA_DIR`, re-running setup will reset them to defaults — acceptable for the current solo-developer scope.
...
- `validateConfig()` is called in `server.ts` inside the existing `if (require.main === module)` guard — it must be the **first statement** inside that block, before `getDb()`, `startScheduler()`, and `app.listen()`, so the checklist appears at the top of terminal output. This guard is correctly triggered by both `npm start` (compiled) and `npm run dev` (tsx) — the checklist is visible in normal development workflow too, not only on production starts.
...
User runs: npm run build && npm start  (or: npm run dev)
    │
    ├── dotenv loads .env
    ├── validateConfig() runs → prints checklist to console  (first thing in require.main block)
    ├── getDb() initialises DB
    ├── startScheduler() starts cron
    └── app.listen() → server on :3001, dashboard at http://localhost:3001
...
- Run `npm run setup` → verify `.env` is written with all 10 variables and correct values
- Run `npm run setup` again → verify overwrite prompt appears and defaults to no (pressing Enter does not overwrite)
- Enter an invalid Client ID format, confirm re-prompt with format hint; fail 3 times, confirm clean abort with no `.env` written
- Start server with all creds set → verify checklist shows all `[✓]`
- Remove `GOOGLE_CLIENT_SECRET` from `.env`, restart → verify `[ ]` for Gmail and `→ Run: npm run setup` hint
- Remove `ENCRYPTION_KEY`, restart → verify hard throw with clear message and no checklist output
- Run `npm run dev` → verify checklist appears (not only on `npm start`)
